...
function initPeripherals()
    ...
    for i=1,#perList,1 do
        ...
        if peripheral.getType(perList[i]) == "fusionReactorLogicAdapter" then
...
function drawDisplayScreen()
	...
	local minVal = math.floor(tonumber(reactor.getCaseTemperature()))
	...
	local minVal = math.floor(tonumber(reactor.getPlasmaTemperature()))
	...
	local minVal = math.floor(tonumber(reactor.getIgnitionTemperature(false)))
	...
	-----------Deuterium Tank---------------------
	draw_text(2, 14, "Deuterium Tank: ", colors.yellow, colors.black)
	local maxVal = reactor.getDeuteriumCapacity()
	local minVal = tonumber(reactor.getDeuterium().amount)
	draw_text(19, 14, minVal.." mb", colors.white, colors.black)
	
	progress_bar(2, 15, displayW-2, minVal, maxVal, colors.red, colors.gray)
	monitor.setBackgroundColour((colours.black))
	
	-----------Tritium Tank---------------------
	draw_text(2, 17, "Tritium Tank: ", colors.yellow, colors.black)
	local maxVal = reactor.getTritiumCapacity()
	local minVal = tonumber(reactor.getTritium().amount)
	draw_text(19, 17, minVal.." mb", colors.white, colors.black)
	
	progress_bar(2, 18, displayW-2, minVal, maxVal, colors.green, colors.gray)
	monitor.setBackgroundColour((colours.black))
	
	-----------Tritium Tank---------------------
	draw_text(2, 20, "DT Fuel: ", colors.yellow, colors.black)
	local maxVal = reactor.getDTFuelCapacity();
	local minVal = tonumber(reactor.getDTFuel().amount);
	draw_text(19, 20, minVal.." mb", colors.white, colors.black)
	
	progress_bar(2, 21, displayW-2, minVal, maxVal, colors.pink, colors.gray)
	monitor.setBackgroundColour((colours.black))
	
	-----------Injection Rate---------------------
	draw_text(2, 22, "Injection Rate: ", colors.yellow, colors.black)
	draw_text(24, 22, reactor.getInjectionRate(), colors.white, colors.black)
	...
	draw_text(2, 23, "Engery Producing RF/T: ", colors.yellow, colors.black)
	draw_text(24, 23, math.floor(tonumber(reactor.getProductionRate()) / 2.5) .. " RF/T", colors.white, colors.black)
	...
	elseif  monitor == nil then 
		...
		while true do
			...
			if displayH == 26 and displayW == 39 then
				if tonumber(reactor.getCaseTemperature()) == nil then
					drawTitle()
